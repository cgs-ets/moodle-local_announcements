    ...
    protected function queue_user_tasks() {
        ...
        foreach ($this->users as $user) {
            ...
            if (!empty($digestposts)) {
                $userdata['announcements'] = $digestposts;
            }

            // MyConnect posts - direct.
            if (isset($this->myconnectuserposts[$user->id])) {
                $userdata['myconnectposts'] = $this->myconnectuserposts[$user->id];
            }

            // MyConnect posts - mentees.
            if (isset($this->myconnectmenteeposts[$user->id])) {
                $userdata['myconnectmenteeposts'] = $this->myconnectmenteeposts[$user->id];
            }

            // Stats.
            $this->logger->log(sprintf("Found %d announcements, %d direct myconnect posts, %d mentee myconnect posts, for %s (%d)",
                count($digestposts),
                isset($this->myconnectuserposts[$user->id]) ? count($this->myconnectuserposts[$user->id]) : 0,
                isset($this->myconnectmenteeposts[$user->id]) ? count(array_merge(...array_values($this->myconnectmenteeposts[$user->id]))) : 0,
                $user->username,
                $user->id,
            ), 2);
